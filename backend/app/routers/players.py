...
def list_players(team_id: Optional[int] = None, db: Session = Depends(get_db)):
    q = db.query(Player).filter(Player.is_active)
    if team_id:
...
def create_player(
    data: PlayerCreate, db: Session = Depends(get_db), _=Depends(require_admin)
):
...
def delete_player(
    player_id: int, db: Session = Depends(get_db), _=Depends(require_admin)
):
    player = db.query(Player).filter(Player.id == player_id).first()
    if not player:
        raise HTTPException(status_code=404, detail="Speler niet gevonden")
