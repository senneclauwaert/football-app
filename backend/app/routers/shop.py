...
from app.schemas import (
    ShopItemOut, ShopItemCreate, ShopItemUpdate,
...
def list_products(db: Session = Depends(get_db)):
    return (
        db.query(ShopItem)
        .filter(ShopItem.is_available == True)
...
def create_product(data: ShopItemCreate, db: Session = Depends(get_db), _=Depends(require_admin)):
...
def update_product(item_id: int, data: ShopItemUpdate, db: Session = Depends(get_db), _=Depends(require_admin)):
    item = db.query(ShopItem).filter(ShopItem.id == item_id).first()
    if not item:
        raise HTTPException(status_code=404, detail="Product niet gevonden")
...
def delete_product(item_id: int, db: Session = Depends(get_db), _=Depends(require_admin)):
    item = db.query(ShopItem).filter(ShopItem.id == item_id).first()
    if not item:
        raise HTTPException(status_code=404, detail="Product niet gevonden")
